...
def get_service(session: Session = Depends(get_session)):
    repository = MateriaRepository(session)
...
# ----------------------
@router.patch(
    "/{materia_id}", response_model=Materia, status_code=status.HTTP_201_CREATED
)
...
async def get_materias(service: MateriaService = Depends(get_service)):
...
async def delete_materia(
    ...
    return service.delete_materia(materia_id)
